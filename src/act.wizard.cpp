...
extern const char *spell_combat[];
extern const char *spell_target[];
extern const char *wound_arr[];
extern const char *spell_detection[];
extern const char *spell_illusion[];
extern const char *spell_manipulation[];
extern const char *spell_health[];
...
ACMD(do_echo)
{
    ...
    if (!*argument)
        ...
        case SCMD_ECHO:
        case SCMD_AECHO:
            if (!PRF_FLAGGED(ch, PRF_QUESTOR) && GET_LEVEL(ch) < LVL_ARCHITECT) {
...
void do_stat_room(struct char_data * ch)
{
    ...
    sprintf(buf, "Extra: [%4d], SpecProc: %s, Flags: %s\r\n", rm->spec,
            (rm->func == NULL) ? "None" : "^CExists^n", buf2);
    send_to_char(buf, ch);
    ...
    if (rm->contents)
    {
        ...
        for (found = 0, j = rm->contents; j; j = j->next_content) {
            if (!CAN_SEE_OBJ(ch, j))
                continue;
            sprintf(buf2, "%s %s", found++ ? "," : "", j->text.name);
            strcat(buf, buf2);
            if (strlen(buf) >= 62) {
                if (j->next_content)
                    send_to_char(strcat(buf, ",\r\n"), ch);
...
void do_stat_object(struct char_data * ch, struct obj_data * j)
{
    ...
    strcat(buf, ", Worn by: ");
    if (j->worn_by)
        strcat(buf, GET_CHAR_NAME(j->worn_by) ? GET_CHAR_NAME(j->worn_by) : GET_NAME(j->worn_by) ? GET_NAME(j->worn_by) : "BROKEN");
    else
        strcat(buf, "Nobody");
    ...
    case ITEM_SPELL_FORMULA:
        sprintf(buf, "(%s) Category: %s, Force: %d, Target: %s, Drain %d%s\r\n"
                "Damage: %s, Type: %s, Effect: %s, For: %s",
                (GET_OBJ_VAL(j, 0) ? "Physical" : "Mana"),
                spell_category[GET_OBJ_VAL(j, 1)],  GET_OBJ_VAL(j, 2),
                spell_target[GET_OBJ_VAL(j, 3)],
                DRAIN_POWER(GET_OBJ_VAL(j, 2)),
                wound_arr[DRAIN_LEVEL(GET_OBJ_VAL(j, 4))],
                wound_arr[GET_OBJ_VAL(j, 5)],
                spells[GET_OBJ_VAL(j, 6)], elements[GET_OBJ_VAL(j, 8)],
                (GET_OBJ_VAL(j, 7) ? "Shaman" : "Mage"));
        break;
    case ITEM_WORKING_GEAR:
        sprintf(buf, "Type: %s", gear_name[GET_OBJ_VAL(j, 0)]);
        break;
    case ITEM_PATCH:
    ...
    case ITEM_PROGRAM:
        ...
        sprintf(buf, "Type: %s, Rating: %d, Size: %d,%sInstalled?: %s",
                program_types[GET_OBJ_VAL(j, 0)], GET_OBJ_VAL(j, 1), GET_OBJ_VAL(j, 2), buf2,
    ...
    case ITEM_FOCUS:
        owner_name = playerDB.GetNameV(GET_OBJ_VAL(j, 9));

        if (GET_OBJ_VAL(j, 0) == FOCI_LOCK)
            sprintf(buf, "Type: Spell lock, Spell: %s, By: %s",
                    (!GET_OBJ_VAL(j, 8) ? "None" :
                     spells[GET_OBJ_VAL(j, 8)]),
                    owner_name? owner_name : "undefined");
        else
            sprintf(buf, "Type: %d, Rating: %d, Spec.: %d, By: %s",
                    GET_OBJ_VAL(j, 0), GET_OBJ_VAL(j, 1), GET_OBJ_VAL(j, 8),
                    owner_name? owner_name : "undefined");
        break;
    ...
    if (j->contains)
    {
        ...
        for (found = 0, j2 = j->contains; j2; j2 = j2->next_content) {
            sprintf(buf2, "%s %s", found++ ? "," : "", j2->text.name);
            strcat(buf, buf2);
            if (strlen(buf) >= 62) {
                if (j2->next_content)
                    send_to_char(strcat(buf, ",\r\n"), ch);
...
void do_stat_character(struct char_data * ch, struct char_data * k)
{
    ...
    sprintf(buf2, " PC '%s'  IDNum: [%5d], In room [%5ld]\r\n",
    ...
    case TRAD_ADEPT:
        ...
        break;
    case TRAD_HERMETIC:
        sprintf(buf, "Tradition: Hermetic, Grade: %d",GET_GRADE(k));
        break;
    case TRAD_SHAMANIC:
        sprinttype(GET_TOTEM(k), totem_types, buf2);
        sprintf(buf, "Tradition: Shamanic, Totem: %s, Grade: %d", buf2,GET_GRADE(k));
        break;
    case TRAD_MUNDANE:
    ...
    sprintf(buf, "B: %d, I: %d, I-Dice: %d, I-Roll: %d, Sus: %d, Foci: %d, TargMod: %d\r\n",
            GET_BALLISTIC(k), GET_IMPACT(k), GET_INIT_DICE(k), GET_INIT_ROLL(k),
            GET_SUSTAINED(k), GET_FOCI(k), GET_TARGET_MOD(k));
    send_to_char(buf, ch);
    ...
    if (k->affected)
    {
        for (aff = k->affected; aff; aff = aff->next) {
            *buf2 = '\0';
            sprintf(buf, "SPL: (%s - %ld) ^c%-21s ", (aff->caster ? "Cast" : "Sust"),
                    aff->sustained_by, spells[aff->type]);
            if (aff->modifier) {
                sprintf(buf2, "%+d to %s", aff->modifier, apply_types[(int) aff->location]);
                strcat(buf, buf2);
            }
            if (aff->bitvector.GetNumSet() > 0) {
                if (*buf2)
                    strcat(buf, ", sets ");
                else
                    strcat(buf, "sets ");
                aff->bitvector.PrintBits(buf2, MAX_STRING_LENGTH,
                                         affected_bits, AFF_MAX);
                strcat(buf, buf2);
            }
            send_to_char(strcat(buf, "\r\n"), ch);
...
void do_stat_mobile(struct char_data * ch, struct char_data * k)
{
    ...
    sprintf(buf, "B: %d, I: %d, I-Dice: %d, I-Roll: %d, Sus: %d, TargMod: %d\r\n",
            GET_BALLISTIC(k), GET_IMPACT(k), GET_INIT_DICE(k), GET_INIT_ROLL(k),
            GET_SUSTAINED(k), GET_TARGET_MOD(k));
    send_to_char(buf, ch);
    ...
    if(mob_index[GET_MOB_RNUM(k)].func)
    {
        ...
        for (index = 0; index < top_of_shopt; index++)
            if (GET_MOB_VNUM(k) == SHOP_KEEPER(index))
                break;
        if (index < top_of_shopt)
            sprintf(buf2, "%s : Shop %ld\r\n", "^CExists^n", SHOP_NUM(index));
        else
            sprintf(buf2, "%s\r\n", "^CExists^n");
    } else
        sprintf(buf2, "None\r\n");
...
ACMD(do_return)
{
    ...
    if (ch->desc && ch->desc->original) {
        ...
        if (PLR_FLAGGED(ch->desc->original, PLR_MATRIX))
            PLR_FLAGS(ch->desc->original).RemoveBit(PLR_MATRIX);
...
ACMD(do_iload)
{
    ...
    sprintf(buf, "%s iloaded object #%d (%s).",
            GET_CHAR_NAME(ch), number, obj->text.keywords);
...
ACMD(do_dc)
{
    ...
    if (*arg == '*') {
        ...
        mudlog(buf, ch, LOG_WIZLOG, TRUE);
        for (d = descriptor_list; d; d = d->next)
            if ((d->connected > 0 && d->connected < CON_SPELL_CREATE) || d->connected > CON_ZEDIT)
                close_socket(d);
        return;
    }
    ...
    if (!*arg || !*to_force)
        ...
        if (!(vict = get_char_vis(ch, arg)))
            ...
            sprintf(buf, "%s forced %s to %s",
                    GET_CHAR_NAME(ch), GET_NAME(vict), to_force);
            mudlog(buf, ch, LOG_WIZLOG, TRUE);
...
ACMD(do_wizwho)
{
    ...
    for (d = descriptor_list; d; d = d->next) {
        if (!d->connected || (d->connected >= CON_SPELL_CREATE && d->connected <= CON_ZEDIT)) {
            if (d->original)
                tch = d->original;
...
void print_zone_to_buf(char *bufptr, int zone, int detailed)
{
    ...
    if (!detailed) {
        sprintf(bufptr, "%3d %-30.30s^n ", zone_table[zone].number,
...
#define GOD_ROOMS_ZONE 0
    ...
    case 9:
        ...
        i = 0;
        *buf = '\0';
        for (temp = vict->spells; temp; temp = temp->next) {
...
ACMD(do_auth)
{
    ...
    if (!*name) {
        send_to_char("Usage: auth <player>\r\n", ch);
        return;
    }
    ...
    if (!PLR_FLAGS(vict).IsSet(PLR_AUTH)) {
        send_to_char("Player is already Authorized.\r\n", ch);
        return;
    }
    PLR_FLAGS(vict).RemoveBit(PLR_AUTH);
    send_to_char("You are now authorized.\r\n", vict);
    send_to_char("Player is now authorized.\r\n", ch);
    sprintf(buf, "%s authed %s.", GET_CHAR_NAME(ch), GET_CHAR_NAME(vict));
    mudlog(buf, ch, LOG_WIZLOG, TRUE);
}
...
ACMD(do_set)
{
    ...
    case 37:
        if (on)
            PRF_FLAGS(vict).SetBits(PRF_COLOR_1, PRF_COLOR_2, ENDBIT);
        else if (off)
            PRF_FLAGS(vict).RemoveBits(PRF_COLOR_1, PRF_COLOR_2, ENDBIT);
        break;
    case 38:
...
#define HOST(rnum) matrix[rnum].vnum
    ...
    for (nr = first; nr < last; nr++) {
        if (ZCMD.if_flag)
            sprintf(buf, "%3d) (if last)", nr);
        else
            sprintf(buf, "%3d) (always) ", nr);
