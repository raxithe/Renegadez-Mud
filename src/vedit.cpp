...
void vedit_disp_menu(struct descriptor_data * d)
{
    ...
    send_to_char(CH, "8) H(^c%d^n) Sp(^c%d^n) Acc(^c%d^n) B(^c%d^n) Ar(^c%d^n)\r\n",
    ...
    for (counter = zone_table[zone].number * 100; counter <= zone_table[zone].top; counter++) {
        ...
        if (realcounter >= 0) {
            *veh = veh_proto[realcounter];
            ...
            if (veh->long_description)
                fprintf(fp, "%s~\n", cleanup(buf2, veh->long_description));
            else
                fprintf(fp, "You see an uncreative vehicle.~\n");
            if (veh->inside_description)
                fprintf(fp, "%s~\n", cleanup(buf2, veh->inside_description));
            else
                fprintf(fp, "You see an uncreative vehicle.~\n");
            if (veh->leave)
                fprintf(fp, "%s~\n", cleanup(buf2, veh->leave));
            else
                fprintf(fp, "leaves~\n");
            if (veh->arrive)
                fprintf(fp, "%s~\n", cleanup(buf2, veh->arrive));
            else
                fprintf(fp, "arrives~\n");
