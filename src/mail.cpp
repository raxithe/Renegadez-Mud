   ...
   header (i.e. the text handed to the player) */
{
    ...
    *(tmstr + strlen(tmstr) - 1) = '\0';
    ...
    sprintf(buf, "^W * * * * World Mail System * * * *\r\n"
            "^YDate: %s\r\n"
            "  ^BTo: %s\r\n"
            "^CFrom: %s^n\r\n\r\n",
            tmstr,
            playerDB.GetNameV(recipient),
            playerDB.GetNameV(header.header_data.from));
                          ...
                          int cmd, char *arg)
{
    ...
    if (GET_NUYEN(ch) < STAMP_PRICE && !IS_SENATOR(ch))
    {
        sprintf(buf, "$n tells you, 'A stamp costs %d nuyen.'\r\n"
                "$n tells you, '...which I see you can't afford.'", STAMP_PRICE);
        act(buf, FALSE, mailman, 0, ch, TO_VICT);
        return;
    }
    if ((recipient = playerDB.GetID(buf)) < 0)
    {
    ...
    if (!IS_SENATOR(ch))
        sprintf(buf, "$n tells you, 'I'll take %d nuyen for the stamp.'\r\n"
                             ...
                             int cmd, char *arg)
{
    ...
    while (has_mail(GET_IDNUM(ch)))
    {
        ...
        obj->photo = read_delete(GET_IDNUM(ch));
