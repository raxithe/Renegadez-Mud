...
ACMD(do_idelete)
{
    int num, counter, i, found = 0;
    if (1) {
        send_to_char(ch, "You're a fucking moron.\r\n");
        return;
    }
    if (!PLR_FLAGGED(ch, PLR_OLC)) {
        send_to_char("Privileged operation.\r\n", ch);
...
ACMD(do_shedit)
{
    ...
    if (real_shop(number) > -1) {
        ...
        shop = new shop_data;
        memset((char *) shop, 0, sizeof(struct shop_data));
        shop->vnum = shop_table[number].vnum;

        shop->num_producing = MAX(MAX_SHOP_OBJ, shop_table[number].num_producing);
        shop->producing = new long[shop->num_producing];
        for (i = 0; i < shop->num_producing; i++)
            if (i < shop_table[number].num_producing)
                shop->producing[i] = shop_table[number].producing[i];
            else
                shop->producing[i] = -1;
        shop->num_producing = shop_table[number].num_producing;

        shop->profit_buy = shop_table[number].profit_buy;
        shop->profit_sell = shop_table[number].profit_sell;

        shop->num_buy_types = shop_table[number].num_buy_types;
        shop->type = new int[NUM_ITEM_TYPES];
        for (i = 0; i < NUM_ITEM_TYPES; i++)
            shop->type[i] = 0;
        for (i = 0; i < shop_table[number].num_buy_types; i++)
            shop->type[shop_table[number].type[i] - 1] = 1;

        shop->temper = shop_table[number].temper;
        shop->bitvector = shop_table[number].bitvector;
        shop->keeper = shop_table[number].keeper;
        shop->with_who = shop_table[number].with_who;

        shop->num_rooms = MAX(MAX_SHOP_ROOMS, shop_table[number].num_rooms);
        shop->in_room = new long[shop->num_rooms];
        for (i = 0; i < shop->num_rooms; i++)
            if (i < shop_table[number].num_rooms)
                shop->in_room[i] = shop_table[number].in_room[i];
            else
                shop->in_room[i] = -1;
        shop->num_rooms = shop_table[number].num_rooms;

        shop->open1 = shop_table[number].open1;
        shop->open2 = shop_table[number].open2;
        shop->close1 = shop_table[number].close1;
        shop->close2 = shop_table[number].close2;
        shop->bankAccount = shop_table[number].bankAccount;
        shop->lastsort = 0;
        if (shop_table[number].no_such_item1)
            shop->no_such_item1 = str_dup(shop_table[number].no_such_item1);
        if (shop_table[number].no_such_item2)
            shop->no_such_item2 = str_dup(shop_table[number].no_such_item2);
        if (shop_table[number].missing_cash1)
            shop->missing_cash1 = str_dup(shop_table[number].missing_cash1);
        if (shop_table[number].missing_cash2)
            shop->missing_cash2 = str_dup(shop_table[number].missing_cash2);
        if (shop_table[number].do_not_buy)
            shop->do_not_buy = str_dup(shop_table[number].do_not_buy);
        if (shop_table[number].message_buy)
            shop->message_buy = str_dup(shop_table[number].message_buy);
        if (shop_table[number].message_sell)
            shop->message_sell = str_dup(shop_table[number].message_sell);
        d->edit_shop = shop;
#if CONFIRM_EXISTING
        ...
        d->edit_shop->num_producing = 0;
        d->edit_shop->producing = new long[MAX_SHOP_OBJ];
        for (i = 0; i < MAX_SHOP_OBJ; i++)
            d->edit_shop->producing[i] = -1;
        d->edit_shop->profit_buy = 1.0;
        d->edit_shop->profit_sell = 1.0;
        d->edit_shop->num_buy_types = 0;
        d->edit_shop->type = new int[NUM_ITEM_TYPES];
        for (i = 0; i < NUM_ITEM_TYPES; i++)
            d->edit_shop->type[i] = 0;
        d->edit_shop->no_such_item1 = str_dup("Sorry, we don't have that.");
        d->edit_shop->no_such_item2 = str_dup("You don't seem to have that.");
        d->edit_shop->missing_cash1 = str_dup("I don't have enough nuyen for that.");
        d->edit_shop->missing_cash2 = str_dup("You don't have enough nuyen!");
        d->edit_shop->do_not_buy = str_dup("We don't buy that.");
        d->edit_shop->message_buy = str_dup("That'll be %d nuyen.");
        d->edit_shop->message_sell = str_dup("Here's your %d nuyen.");
        d->edit_shop->temper = 0;
        d->edit_shop->bitvector = 0;
        d->edit_shop->keeper = -1;
        d->edit_shop->with_who = 0;
        d->edit_shop->num_rooms = 0;
        d->edit_shop->in_room = new long[MAX_SHOP_ROOMS];
        for (i = 0; i < MAX_SHOP_ROOMS; i++)
            d->edit_shop->in_room[i] = -1;
        d->edit_shop->open1 = 0;
        d->edit_shop->open2 = 0;
        d->edit_shop->close1 = 24;
        d->edit_shop->close2 = 0;
        d->edit_shop->bankAccount = 0;
        d->edit_shop->lastsort = 0;
        d->edit_mode = SHEDIT_CONFIRM_EDIT;
    }
}
...
ACMD(do_sdelete)
{
    int num, count, count2,i, found = 0;
    ...
    if (!*buf) {
        send_to_char("Usage: sdelete <vnum>\r\n", ch);
        return;
    }
    ...
    for (count = 0; count <= top_of_zone_table; count++) {
        if ((SHOP_NUM(num) >= (zone_table[count].number * 100)) &&
                (SHOP_NUM(num) <= (zone_table[count].top))) {
            found = 1;
            break;
        }
    }
    ...
    for (i = 0; i < 5; i++)
        if (zone_table[count].editor_ids[i] == GET_IDNUM(ch))
            break;
    if ((i >= 5) && (!access_level(ch, LVL_ADMIN))) {
        send_to_char("Sorry, you don't have access to edit this zone.\r\n", ch);
        return;
    ...
    if (!(access_level(ch, LVL_EXECUTIVE) || PLR_FLAGGED(ch, PLR_EDCON)) && zone_table[count].connected) {
        send_to_char("You can't delete shops from a connected zone.\r\n", ch);
        return;
    }

    ch->player_specials->saved.zonenum = zone_table[count].number;
    num = real_shop(num);

    // fix the shop rnums
    for (count = num; count < (top_of_shopt - 1); count++) {
        SHOP_NUM(count) = SHOP_NUM(count+1);
        if (shop_table[count].no_such_item1)
            delete [] shop_table[count].no_such_item1;
        shop_table[count].no_such_item1 = str_dup(shop_table[count+1].no_such_item1);
        if (shop_table[count].no_such_item2)
            delete [] shop_table[count].no_such_item2;
        shop_table[count].no_such_item2 = str_dup(shop_table[count+1].no_such_item2);
        if (shop_table[count].do_not_buy)
            delete [] shop_table[count].do_not_buy;
        shop_table[count].do_not_buy = str_dup(shop_table[count+1].do_not_buy);
        if (shop_table[count].missing_cash1)
            delete [] shop_table[count].missing_cash1;
        shop_table[count].missing_cash1 = str_dup(shop_table[count+1].missing_cash1);
        if (shop_table[count].missing_cash2)
            delete [] shop_table[count].missing_cash2;
        shop_table[count].missing_cash2 = str_dup(shop_table[count+1].missing_cash2);
        if (shop_table[count].message_buy)
            delete [] shop_table[count].message_buy;
        shop_table[count].message_buy = str_dup(shop_table[count+1].message_buy);
        if (shop_table[count].message_sell)
            delete [] shop_table[count].message_sell;
        shop_table[count].message_sell = str_dup(shop_table[count+1].message_sell);
        SHOP_BUYPROFIT(count) = SHOP_BUYPROFIT(count+1);
        SHOP_SELLPROFIT(count) = SHOP_SELLPROFIT(count+1);
        SHOP_TEMPER(count) = SHOP_TEMPER(count+1);
        SHOP_BITVECTOR(count) = SHOP_BITVECTOR(count+1);
        SHOP_TRADE_WITH(count) = SHOP_TRADE_WITH(count+1);
        SHOP_OPEN1(count) = SHOP_OPEN1(count+1);
        SHOP_CLOSE1(count) = SHOP_CLOSE1(count+1);
        SHOP_OPEN2(count) = SHOP_OPEN2(count+1);
        SHOP_CLOSE2(count) = SHOP_CLOSE2(count+1);
        SHOP_KEEPER(count) = SHOP_KEEPER(count+1);
        if (shop_table[count].producing)
            delete [] shop_table[count].producing;
        shop_table[count].num_producing = shop_table[count+1].num_producing;
        shop_table[count].producing = new long[shop_table[count].num_producing];
        for (count2 = 0; count2 < shop_table[count].num_producing; count2++)
            SHOP_PRODUCT(count, count2) = SHOP_PRODUCT(count + 1, count2);

        if (shop_table[count].type)
            delete [] shop_table[count].type;
        shop_table[count].num_buy_types = shop_table[count+1].num_buy_types;
        shop_table[count].type = new int[shop_table[count].num_buy_types];
        for (count2 = 0; count2 < shop_table[count].num_buy_types; count2++)
            SHOP_BUYTYPE(count, count2) = SHOP_BUYTYPE(count + 1, count2);

        shop_table[count].num_rooms = shop_table[count+1].num_rooms;
        if (shop_table[count].in_room)
            delete [] shop_table[count].in_room;
        shop_table[count].in_room = new long[shop_table[count].num_rooms];
        for (count2 = 0; count2 < shop_table[count].num_rooms; count2++)
            SHOP_ROOM(count, count2) = SHOP_ROOM(count + 1, count2);
    }
