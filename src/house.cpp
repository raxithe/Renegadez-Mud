...
bool House_load(struct house_control_rec *house)
{
    ...
    for (int i = 0; i < MAX_GUESTS; i++)
    {
        ...
        if (house->guests[i] == 0 && !house->num_of_guests)
            house->num_of_guests = i;
    }
...
void House_save_control(void)
{
    ...
    for (llord = landlords; llord; llord = llord->next) {
        ...
        for (room = llord->rooms; room; room = room->next)
            fprintf(fl, "%ld %ld %d %d %s %ld %d %ld\n", room->vnum, room->key, room->exit_num, room->mode,
                    room->name, room->owner, room->num_of_guests, room->owner ? room->date : 0);
    }
...
void hcontrol_list_houses(struct char_data *ch)
{
    ...
    for (struct landlord *llord = landlords; llord; llord = llord->next)
        for (struct house_control_rec *house = llord->rooms; house; house = house->next)
            if (house->owner)
            {
                strcpy(own_name, playerDB.GetNameV(house->owner));
                sprintf(buf, "%7ld %7ld    %2d    %-12s\r\n",
                        house->vnum, world[house->atrium].number, house->num_of_guests, CAP(own_name));
                send_to_char(buf, ch);
            }
}
...
ACMD(do_house)
{
    ...
    if (!ROOM_FLAGGED(IN_ROOM(ch), ROOM_HOUSE))
        ...
        for (j = 0; j < i->num_of_guests; j++)
            if (i->guests[j] == id) {
                for (; j < i->num_of_guests; j++)
                    i->guests[j] = i->guests[j + 1];
                i->num_of_guests--;
                House_save_control();
...
bool House_can_enter(struct char_data *ch, vnum_t house)
{
    ...
    if (GET_IDNUM(ch) == room->owner)
        return TRUE;
    for (j = 0; j < room->num_of_guests; j++)
